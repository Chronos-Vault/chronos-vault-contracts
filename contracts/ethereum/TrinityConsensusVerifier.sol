 ...
 * @notice Minimal interface to query vault types from TrinityConsensusVerifier
 */
...
/**
 * @title Trinity Protocol™ v3.2 - Multi-Chain Consensus Verification System  
 ...
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * 
 * 🏦 SIMPLE EXPLANATION - WHAT TRINITY PROTOCOL IS:
 * 
 * Think of it like a BANK VAULT with 3 different security guards:
 *   • Ethereum guard (Arbitrum) - Primary security layer
 *   • Solana guard - Fast monitoring and verification  
 *   • TON guard - Emergency backup and recovery
 * 
 * To open the vault and authorize an operation, you need 2 out of 3 guards to agree.
 * This provides mathematical security: Attack probability ~10^-18 (requires compromising  
 * 2 independent blockchains simultaneously).
 * 
 ...
 * ✅ Circuit breaker protection against anomalies and attacks
 * ✅ Distributed validator network preventing single points of failure
 * 
 * ❌ WHAT THIS CONTRACT IS **NOT**:
 * ❌ NOT a cross-chain token bridge (does NOT move tokens between chains)
 * ❌ NOT LayerZero or Wormhole (completely different technology)
 * ❌ NOT a DEX or token swap protocol
 * ❌ NOT for transferring assets from Ethereum → Solana → TON
 * ❌ NOT moving money between chains - it's PROVING CONSENSUS on one chain
 * 
 * 📝 CORRECT USE CASE EXAMPLE:
 * User deposits 100 ETH into a vault on Arbitrum. Later, they want to withdraw.
 * Trinity Protocol verifies 2-of-3 consensus from validators monitoring all three  
 * networks (Arbitrum, Solana, TON). Once consensus is achieved, funds are released  
 * from the SAME vault on Arbitrum where they were deposited.
 * 
 * This is NOT bridging - it's consensus-verified authorization for operations.
 * 
...
contract TrinityConsensusVerifier is ReentrancyGuard {
    ...
    // SECURITY FIX v3.3 #1: Validator Rotation with 2-of-3 Consensus
    struct ValidatorRotationProposal {
        uint8 chainId;
        address oldValidator;
        address newValidator;
        bool isAddition; // true = add validator, false = remove validator
        uint256 proposedAt;
        uint8 confirmations;
        mapping(address => bool) confirmedBy; // Track which validators confirmed
    }
    mapping(bytes32 => ValidatorRotationProposal) public validatorRotationProposals;
    uint256 public constant ROTATION_PROPOSAL_EXPIRY = 7 days;
    
    // SECURITY FIX v3.3 #2: Merkle Root Update with 2-of-3 Consensus
    ...
    mapping(uint8 => mapping(bytes32 => MerkleRootProposal)) public merkleRootProposals; // chainId => proposalId => proposal
    uint256 public constant MERKLE_PROPOSAL_EXPIRY = 24 hours;
    ...
    // SECURITY FIX v3.3: Validator rotation events
    event ValidatorRotationProposed(
    ...
    event ValidatorRotationConfirmed(
        bytes32 indexed proposalId,
        address indexed confirmedBy,
        uint8 confirmations
    );
    
    event ValidatorRotationExecuted(
        bytes32 indexed proposalId,
        uint8 indexed chainId,
        address oldValidator,
        address newValidator,
    ...
    // SECURITY FIX v3.3: Merkle root consensus events
    event MerkleRootProposed(
        bytes32 indexed proposalId,
        uint8 indexed chainId,
        bytes32 merkleRoot,
        uint256 nonce,
    ...
    event MerkleRootConsensusReached(
        bytes32 indexed proposalId,
        uint8 indexed chainId,
        bytes32 merkleRoot,
        uint256 nonce
     ...
     * 🔐 SECURITY FIX v3.3 #1: VALIDATOR ROTATION WITH 2-OF-3 CONSENSUS
     * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     * Allows adding or removing validators with 2-of-3 validator consensus.
     * Fixes vulnerability where compromised validator keys cannot be revoked.
     * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     */
    
    /**
     * @notice Propose adding or removing a validator (requires 2-of-3 validator consensus)
     * @param chainId Chain to rotate validator for (1=ETH, 2=SOL, 3=TON)
     * @param oldValidator Validator to remove (address(0) if adding new validator)
     * @param newValidator Validator to add (address(0) if removing validator)
     * @param isAddition true = add validator, false = remove validator
     */
    function proposeValidatorRotation(
        ...
        if (!authorizedValidators[chainId][msg.sender]) revert Unauthorized();
        
        // Validate rotation parameters
        if (isAddition) {
            if (newValidator == address(0)) revert Errors.ZeroAddress();
            if (authorizedValidators[chainId][newValidator]) revert Errors.ValidatorAlreadyAuthorized();
        } else {
            if (oldValidator == address(0)) revert Errors.ZeroAddress();
            if (!authorizedValidators[chainId][oldValidator]) revert Errors.ValidatorNotFound();
        }
        ...
        // Initialize proposal
        ValidatorRotationProposal storage proposal = validatorRotationProposals[proposalId];
        ...
        proposal.confirmations = 1; // Proposer automatically confirms
        proposal.confirmedBy[msg.sender] = true;
        
        emit ValidatorRotationProposed(proposalId, chainId, oldValidator, newValidator, isAddition, msg.sender);
        emit ValidatorRotationConfirmed(proposalId, msg.sender, 1);
    }
    
    /**
     * @notice Confirm a validator rotation proposal (requires 2-of-3 consensus)
     * @param proposalId The proposal ID to confirm
     */
    function confirmValidatorRotation(bytes32 proposalId) external {
        ValidatorRotationProposal storage proposal = validatorRotationProposals[proposalId];
        
        if (proposal.proposedAt == 0) revert Errors.ProposalNotFound();
        if (block.timestamp > proposal.proposedAt + ROTATION_PROPOSAL_EXPIRY) revert Errors.ProposalExpired();
        if (!authorizedValidators[proposal.chainId][msg.sender]) revert Unauthorized();
        if (proposal.confirmedBy[msg.sender]) revert Errors.AlreadyConfirmed();
        ...
        if (proposal.confirmations >= 2) {
            if (proposal.isAddition) {
                authorizedValidators[proposal.chainId][proposal.newValidator] = true;
            } else {
                authorizedValidators[proposal.chainId][proposal.oldValidator] = false;
            }
            
            emit ValidatorRotationExecuted(
                proposalId,
                proposal.chainId,
                proposal.oldValidator,
                proposal.newValidator,
                proposal.isAddition
            );
        }
    }
     ...
     * 🔐 SECURITY FIX v3.3 #2: MERKLE ROOT UPDATE WITH 2-OF-3 CONSENSUS
     * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     * Requires 2-of-3 validator consensus to update Merkle roots.
     * Fixes vulnerability where single compromised validator can submit malicious roots.
     * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     */
    
    /**
     * @notice Propose a Merkle root update (requires 2-of-3 validator consensus)
    ...
    function proposeMerkleRootUpdate(
        ...
        // Initialize proposal
        MerkleRootProposal storage proposal = merkleRootProposals[chainId][proposalId];
        ...
        emit MerkleRootProposed(proposalId, chainId, merkleRoot, nonce, msg.sender);
        emit MerkleRootConfirmed(proposalId, chainId, msg.sender, 1);
    }
    
    /**
     * @notice Confirm a Merkle root update proposal (requires 2-of-3 consensus)
     * @param chainId Chain ID for the proposal
     * @param proposalId The proposal ID to confirm
     */
    function confirmMerkleRootUpdate(
        uint8 chainId,
        bytes32 proposalId
    ) external {
        if (chainId < 1 || chainId > 3) revert InvalidChainID();
        ...
        if (proposal.proposedAt == 0) revert Errors.ProposalNotFound();
        if (block.timestamp > proposal.proposedAt + MERKLE_PROPOSAL_EXPIRY) revert Errors.ProposalExpired();
        if (proposal.confirmedBy[msg.sender]) revert Errors.AlreadyConfirmed();
        
        // Validate nonce still valid
        if (proposal.nonce != merkleRootNonce[chainId] + 1) revert InvalidNonceSequence();
        
        // Add confirmation
        proposal.confirmations++;
        proposal.confirmedBy[msg.sender] = true;
        
        emit MerkleRootConfirmed(proposalId, chainId, msg.sender, proposal.confirmations);
        
        // Execute if 2-of-3 consensus reached
        if (proposal.confirmations >= 2) {
            trustedMerkleRoots[chainId] = proposal.merkleRoot;
            merkleRootNonce[chainId] = proposal.nonce;
            
            emit MerkleRootConsensusReached(proposalId, chainId, proposal.merkleRoot, proposal.nonce);
            emit MerkleRootUpdated(chainId, proposal.merkleRoot, proposal.nonce, msg.sender);
        }
    }
    
    /**
     * @dev FIX #3: Update trusted Merkle roots with NONCE-BASED replay protection
     * @notice DEPRECATED: Use proposeMerkleRootUpdate() + confirmMerkleRootUpdate() instead
     * @dev This function is kept for backward compatibility but bypasses 2-of-3 consensus
     * @param chainId Chain to update root for (1=ETH, 2=SOL, 3=TON)
